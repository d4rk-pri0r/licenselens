...
| WP2 | 2 | Unknown entitlement, detection unknown/watched, error≠partial, headline fraction, dist-bar split, "fully working" wording | WP1 | Fable + GLM | **verified** (engineering) | four unknown caps → entitlement_unknown=4; missing ingestion Not assessed; parity error watched_by=None; hero "3 of 8 … met all assessed criteria (38%)"; 59 tests | `.omo/evidence/tv-2/ATLAS-VERDICT.md` |
| WP3 | 3 | Wizard autoescape fix + hostile-text regression + real browser click; hotel-room before/after dirs | WP1–2 for full release truth | Fable in-process | **verified** (engineering; public-package matrix still pending) | Playwright clicks "Run the offline demo" and reaches a report; Origin:null + CSRF still required; 18 UI tests | `e33f4ba` |
| WP4 | 4 | Demo after-overlay coherent (legacy-auth block added) | WP1 | Fable in-process | **verified** (engineering) | after overlay: SD off + legacy OK, no new gap; live demo before/after 170 findings, new_gaps=[] | `7b34341` |
...
| WP6 | 6 | Three findings → one work package template; action-plan export corrections | WP2 | Fable (template) | template **ready**; export corrections and customer cycle **awaiting external evidence** | `docs/work-package-template.md` (excluded from public nav) | invitations / customer still Joe |
...
- 2026-09-07 — WP3 wizard PASS (Playwright click). WP4 overlay PASS (no invented legacy-auth gap).
