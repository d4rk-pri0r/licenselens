...
def test_detection_realization_rows() -> None:
    ...
    assert all(row["ingesting"] is not None or row["watched_by"] is None for row in rows), (
...
def test_detection_realization_unknowns_are_not_false() -> None:
    ...
    parity_error = _detection_finding("sen-rule-telemetry-parity", FindingStatus.ERROR, evidence={})
    errored = build_detection_realization(ScanResult(**base, findings=[ingestion_ok, parity_error]))
    ...
    gapped = build_detection_realization(ScanResult(**base, findings=[ingestion_ok, parity_gap]))
    ...
    parity_ok = _detection_finding("sen-rule-telemetry-parity", FindingStatus.OK, evidence={})
    watched = build_detection_realization(ScanResult(**base, findings=[ingestion_ok, parity_ok]))
